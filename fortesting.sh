...
username: customer_$customerno
password:" |\
msmtp $(< /tmp/cuse-mail) &) |\
htpasswd -in customer_$customerno)" >> myusers
echo 'sed "/$customerno/d" myusers' | at now + 3 days &>/dev/null # in 3 days delete the line in .myusers associated with that customer

else

# this achieves a similar result, except that htpasswd is in branch one
openssl rand -base64 32 |\
tee >(htpasswd -ci myusers customer &>/dev/null) |\
xargs echo "
Thank you for your purchase. The files for download are available at https://filmsbytom.com/shopfiles/ . Please use the username and password below. This password will expire in 3 days.
username: customer
password:" |\
msmtp $(< /tmp/cuse-mail)
echo "sed '/customer:/d' myusers" | at now + 3 days &>/dev/null
